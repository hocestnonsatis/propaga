...
Supported variable selectors: `input_order`, `first_fail` / `most_constrained`, `anti_first_fail` / `least_constrained` (largest domain), `smallest` / `largest` (by domain min/max; floats use scaled bounds), `max_regret` (ints: min-to-next gap; floats: bound width proxy), `dom_w_deg` / `occurrence` / `degree` (W-DEG approximation), `activity` / `vsids`.
