...
| `set_in`, `set_le`, `set_lt`, `set_diff`, `set_symdiff` | Supported | Native `set_diff` / `set_symdiff`; `set_le`/`set_lt` are MiniZinc sorted-list lex order (not ⊆) |
...
| `float_ceil` / `float_floor` / `float_round` | Drop integer images whose preimages are emptied by holes (typically endpoint-only); constant domains collapse to fixed; fixed integer images reverse-project onto the input; spans above 10 000 integers keep hole-free bounds |
