...
pub fn set_le_reif(model: &mut Model, left: VariableId, right: VariableId, reif: VariableId) {
    // Half-reif: reif=true posts lex ≤; reif stays free when lex is undetermined.
    // When lex is impossible, force reif=false; when inevitable, force reif=true.
    model.set_lex_le_reif(left, right, reif);
}

/// Posts `reif <=> left <_lex right`.
pub fn set_lt_reif(model: &mut Model, left: VariableId, right: VariableId, reif: VariableId) {
    model.set_lex_lt_reif(left, right, reif);
}
